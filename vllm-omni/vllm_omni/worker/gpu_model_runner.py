...
class OmniGPUModelRunner(GPUModelRunner):
    ...
    def _update_states(self, scheduler_output: "SchedulerOutput") -> None:
        ...
        for req_id in scheduler_output.finished_req_ids:
            ...
            # 🎯 [조언] 진짜 종료 시점은 Stage 2의 마지막 조각이 끝났을 때입니다.
            # Stage 0, Stage 1, Stage 2의 중간 조각들은 모두 통과시킵니다.
            if "_stage_2_" in req_id and "_FINAL" in req_id:
                session_id = req_id.split('_stage_')[0]
                if hasattr(self.model, "reset_session_state"):
                    self.model.reset_session_state(session_id)
                    #logger.info(f"🏁 [FINAL RESET] {session_id}의 모든 프로세스 완료. 메모리 정리.")
            else:
                # 그 외의 경우(Stage 0 종료 등)에는 절대 리셋을 부르지 않습니다.
                continue
    ...
    def _decode_and_store_request_payloads(self, scheduler_output: "SchedulerOutput") -> None:
        ...
        try:
            ...
            # -------------------------------------------------------------
            # 2. [추가된 로직] 기존 요청(Cached Request) 업데이트 처리
            #    우리가 output.py에 추가한 'additional_info_updates' 필드를 확인합니다.
            # -------------------------------------------------------------
            updates = getattr(scheduler_output, "additional_info_updates", {})
            
            if updates:
                for req_id, payload_info in updates.items():
                    ...
                    # [핵심] 109개를 115개, 120개로 강제 업데이트!
                    if info_dict and req_id in self.requests:
                        setattr(self.requests[req_id], "additional_information_cpu", info_dict)
                        # logger.info(f"🔥 [UPDATE] RID {req_id} updated via SchedulerOutput!")
        except Exception as e:
        ...
        try:
            ...
            if hasattr(self.model, "has_postprocess") and self.model.has_postprocess:
                for req_index, req_id in enumerate(self.input_batch.req_ids):
                    ...
                    if req_infos is not None:
                        session_id = req_id.split('_stage_')[0]
                        req_infos["omni_sid"] = session_id
                    start_offset = int(self.query_start_loc.cpu[req_index])
                    sched_tokens = int(num_scheduled_tokens_np[req_index])
    ...
    def _preprocess(
        ...
        if hasattr(self.model, "has_preprocess") and self.model.has_preprocess:
            ...
            for req_index, req_id in enumerate(self.input_batch.req_ids):
                ...
                ai_mode = "G"
                if req_state and hasattr(req_state, "sampling_params"):
                    params = req_state.sampling_params.extra_args
                ...
                #logger.debug(f"🎯 [Position][Before Preprocess] Req: {req_id} | Ori Value: {positions[-1][-1]}")
                req_input_ids, req_embeds, update_dict = self.model.preprocess(
                    input_ids=input_ids[s:e], input_embeds=inputs_embeds[s:e], positions=positions, req_id=req_id, runner=runner, ai_mode=ai_mode, **req_infos
                )
    ...
    def _model_forward(
        ...
        **model_kwargs: dict[str, Any],
        ...
        # 🎯 [핵심] positions 텐서 수정 로직 시작
        if hasattr(self.model, "stg1_current_pos"):
            # vLLM은 배치를 한꺼번에 처리하므로 query_start_loc를 이용해 각 리퀘스트의 위치를 찾습니다.
            ...
            for i, req_id in enumerate(self.input_batch.req_ids):
                if '_stage_1_' in str(req_id):
                    ...
                    # 2. 이 세션에 저장된 진짜 포지션이 있는지 확인
                    if session_id in self.model.stg1_current_pos:
                        # 저장된 positions는 텐서 형태일 것이므로 마지막 값을 가져옵니다.
                        saved_positions = self.model.stg1_current_pos[session_id][:, -inputs_embeds.shape[0]:]
                ...
                if req_state and hasattr(req_state, "sampling_params"):
                    params = req_state.sampling_params.extra_args
                    chunk_unit = params["chunk_unit"]
                    overlap_size = params["overlap_size"]
                    #logger.info(f"chunk_unit: {chunk_unit} | overlap_size: {overlap_size}")
                    model_kwargs_extra["chunk_unit"] = chunk_unit
                    model_kwargs_extra["overlap_size"] = overlap_size
                        
        #logger.debug(f"🎯 [Position] Req: {req_id} | SK Value: {saved_positions[-1][-1]}, Ori Value: {positions[-1][-1]}")
        ...
        # 🎯 2. [사용자님 제안 반영] 연산 성공 후 사후 저장 로직
        if model_output is not None and hasattr(self.model, "stg1_current_pos") and hasattr(self.model, "stg1_last_hidden"):
            query_start_loc = self.query_start_loc.cpu
            for i, req_id in enumerate(self.input_batch.req_ids):
                if '_stage_1_' in str(req_id):
                    session_id = req_id.split('_stage_1_')[0] if '_stage_1_' in req_id else "default_session"
                    if session_id not in self.model.stg1_current_pos:
                        last_positions = saved_positions
                    else:
                        last_positions = self.model.stg1_current_pos[session_id]
                    last_val_column = last_positions[:, -1:] 
                    next_val_column = last_val_column + 1
                    self.model.stg1_current_pos[session_id] = torch.cat(
                        [last_positions, next_val_column], 
                        dim=-1
                    )
                    all_hidden = model_output.text_hidden_states
                    last_hidden = all_hidden[-1, :]
                    logger.debug(f"[Runner] last_hidden Shape: {last_hidden.shape} | ({last_hidden})")
                    self.model.stg1_last_hidden[session_id] = all_hidden[-1, :].detach()

        # Cache model output so later sample_tokens can consume multimodal results.
        self._omni_last_model_output = model_output
        # 🎯 캐시된 최종 출력물에서 데이터 확인
        if self._omni_last_model_output is not None:
            mm_out = getattr(self._omni_last_model_output, "multimodal_outputs", None)
            if mm_out:
                keys = list(mm_out.keys())
                if "code_predictor_codes" in mm_out:
                    shape = mm_out["code_predictor_codes"].shape
        return model_output
