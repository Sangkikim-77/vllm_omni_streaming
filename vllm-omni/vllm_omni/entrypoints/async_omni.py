...
class AsyncOmni(OmniBase):
    ...
    def _run_output_handler(self) -> None:
        ...
        # 🎯 스테이지별 전용 수집기 정의
        async def stage_harvester(stage_id: int, stage: Any):
            try:
                while True:
                    # 1. 해당 스테이지에서만 물건 꺼내기 (Non-blocking)
                    result = stage.try_collect()
                    
                    if result is None:
                        # 2. 물건 없으면 아주 잠깐 양보 (0.001초)
                    ...
                    # 5. 배달 (중요: 다른 스테이지 태스크와 독립적으로 작동)
                    # 만약 여기서 정체가 발생해도 해당 스테이지 배달부만 멈춤
                ...
                logger.exception(f"❌ [Harvester-{stage_id}] failed: {e}")

        # 🚀 각 스테이지별로 독립된 배달부(Task) 기동
        self.harvester_tasks = [
            asyncio.create_task(stage_harvester(i, stage)) 
            for i, stage in enumerate(stage_list)
        ]
        
        # 기존 호환성을 위해 하나만 할당하거나 전체 관리자로 설정
        self.output_handler = self.harvester_tasks[0]


    def _run_output_handler1(self) -> None:
        if self.output_handler is not None:
            return
