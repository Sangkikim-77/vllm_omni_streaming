...
class OmniStage:
    ...
    def drain(self):
        ...
        try:
            ...
            pass
        if count > 0:
            logger.info(f"🧹 [Drain] Stage-{self.stage_id} 큐에서 {count}개 쓰레기 제거.")

    def selective_drain(self, target_ids: list[str]):
        """확정된 target_ids에 해당하는 태스크만 큐에서 제거하고 SHM을 해제합니다."""
        if self._in_q is None or not target_ids:
            return

        target_set = set(target_ids) # 검색 속도를 위해 set으로 변환
        retained_items = []
        removed_count = 0
        shm_count = 0

        try:
            # 1. 큐의 모든 내용을 일단 다 꺼냅니다.
            while not self._in_q.empty():
                try:
                    ...
                    if rid in target_set:
                        # [Destroy] 타겟이면 SHM 물리 삭제 진행
                        logger.info(f"🎯 {rid} in Target")
                        shm_meta = item.get("connector_metadata", {}).get("shm")
                        if shm_meta and "name" in shm_meta:
                            try:
                                s = shared_memory.SharedMemory(name=shm_meta["name"])
                                s.close()
                                s.unlink()
                                shm_count += 1
                            except: pass
                        else:
                            logger.info(f"shm_meta None")
                        removed_count += 1
                    else:
                        # [Keep] 내꺼 아니면 보관함에 잠시 킵
                        retained_items.append(item)
                except queue.Empty:
                    break

            # 3. 지우지 말아야 할(남의 턴/남의 유저) 데이터는 다시 큐에 넣어줍니다.
            for retained_item in retained_items:
                self._in_q.put(retained_item)

        except Exception as e:
            logger.error(f"❌ [SelectiveDrain] 오류: {e}")

        if removed_count > 0:
            logger.info(f"🎯 [Target-Purge] Stage-{self.stage_id}: {removed_count}개 제거, {shm_count}개 SHM 해제, {len(retained_items)}개 유지.")
...
async def _stage_worker_async(
    ...
    async def generation_single_request(task: dict[str, Any]):
        ...
        try:
            ...
            # [추가] 최신 임베딩 정보를 공유 저장소에 기록
            if isinstance(ein, list) and len(ein) > 0:
                target_ein = ein[0]
                # OmniTokensPrompt나 딕셔너리에서 additional_information 추출
                new_info = getattr(target_ein, 'additional_information', {})
                if not new_info and isinstance(target_ein, dict):
                    new_info = target_ein.get('additional_information', {})
                    
                if new_info:
                    # 🚨 엔진 클라이언트를 거치지 않고 클래스 정적 변수에 바로 저장
                    GPUARModelRunner._STAGING_METADATA[rid] = new_info
            if ein is None or _rx_metrics is None:
                raise RuntimeError(
                    f"[Stage-{stage_id}] Missing connector payload for request {rid}. "
                    "Ensure connectors are configured for all incoming edges."
            ...
            #logger.warning(f"🛑 [Worker] Task for {rid} was explicitly cancelled.")
            raise
        except Exception as e:
            ...
            active_generation_tasks.pop(rid, None)
    ...
    while True:
        try:
            ...
            #logger.debug(f"📥 [Worker Loop] Task Received! Type: {task.get('type')}, RID: {task.get('request_id')}")
            if task_type == OmniStageTaskType.SHUTDOWN:
                #logger.debug("Received shutdown signal")
                stage_engine.shutdown()
                break
            elif task_type == OmniStageTaskType.ABORT:
                target_session_id = task["request_id"]
                aborted_sessions.add(target_session_id)
                targets = [rid for rid in active_generation_tasks.keys() if target_session_id in rid]
                for rid in targets:
                    task_obj = active_generation_tasks.pop(rid, None)
                    if task_obj:
                        task_obj.cancel()
                        asyncio.create_task(stage_engine.abort(rid))
            elif task_type == OmniStageTaskType.GENERATE:
                
                curr_session = rid.split("_stage_")[0]
                if last_processed_session is None:
                    last_processed_session = curr_session
                elif last_processed_session is not None and curr_session != last_processed_session:
                    logger.info(f"♻️ New session detected: {curr_session}. Clearing abort list.")
                    aborted_sessions.clear()
                    last_processed_session = curr_session

                if any(sid in rid for sid in aborted_sessions):
                    logger.info(f"🗑️ [Skip] Dropping queued task for aborted session: {rid}")
                    continue

                gen_task = asyncio.create_task(generation_single_request(task))
                active_generation_tasks[rid] = gen_task
                gen_task.add_done_callback(lambda t: active_generation_tasks.pop(rid, None))
            else:
        ...
        for rid, output, _gen_ms, _metrics in zip(
            ...
            if rid not in active_generation_tasks and task_type != OmniStageTaskType.GENERATE:
                #logger.warning(f"🗑️ [Worker] Discarding output for aborted request: {rid}")
                continue
            try:
                r_outputs = [output]
                use_shm, payload = maybe_dump_to_shm(r_outputs, shm_threshold_bytes)
