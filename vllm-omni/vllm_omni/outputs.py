...
class OmniModelRunnerOutput(ModelRunnerOutput):
    ...
    def __init__(
        ...
        *,
        ...
        additional_information: dict[str, dict] | None = None,
        **kwargs,
    ):
...
class OmniRequestOutput:
    ...
    def from_pipeline(
        ...
        finished: bool = True,  # ★ finished 인자 추가 (기본값 True)
        ...
        """
        # 1. 외부에서 request_id를 주면 그걸 쓰고, 안 주면 내부 객체에서 꺼내 씁니다.
        final_req_id = request_id if request_id is not None else getattr(request_output, "request_id", "")
        return cls(
