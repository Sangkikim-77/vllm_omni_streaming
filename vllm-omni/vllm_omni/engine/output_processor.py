...
class OmniRequestState(RequestState):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.stage0_next_send_idx: int = -1
    ...
    def make_request_output(
        ...
        """
        [수정 포인트]
        1. ro 객체가 생성된 직후 mm_accumulated를 큐에 실어 보냅니다.
        2. 보낸 데이터는 self.mm_accumulated = None으로 비웁니다 (Flush).
        """
        ...
        # [여기가 핵심입니다]
        ro = self._new_request_output(request_id, outputs, finished, kv_transfer_params)
        
        if ro and self.mm_accumulated is not None and not finished:
            # [핵심] 현재까지 쌓인 전체 데이터를 ro에 담음
            ro.multimodal_output = self.mm_accumulated

            #if "stage_0" in request_id and "stage_1" not in request_id and "stage_2" not in request_id:
            #    mm = self.mm_accumulated
                # 🔍 DEBUG: mm key 확인
                #logger.error(
                #    f"[MM-DEBUG] request_id={request_id} "
                #    f"mm_keys={list(mm.keys())} "
                #    f"mm_types={{k: type(v) for k, v in mm.items()}}"
                #)
            #    tensor = mm["0"]
            #    self.stage0_next_send_idx = tensor.shape[0]
                #logger.info(f"self.stage0_next_send_idx: {self.stage0_next_send_idx}")
            # [핵심] vLLM의 리턴을 기다리지 않고, 여기서 직접 큐에 주입!
        ...
        """
        is_stage0 = (
            "stage_0" in ro.request_id
            and "stage_1" not in ro.request_id
            and "stage_2" not in ro.request_id
        )
        # ---------- FINISH: chunked mm flush ----------
        if is_stage0 and finished and self.mm_accumulated is not None:
            

            mm = self.mm_accumulated
            self.mm_accumulated = None

            thinker_sequence_embeds = mm.get("0")
            thinker_hidden_states = mm.get("24")

            logger.info(f"thinker_sequence_embeds: {thinker_sequence_embeds.shape[0]}")
            

            T1 = thinker_sequence_embeds.shape[0]
            T2 = thinker_hidden_states.shape[0]
            assert T1 == T2, (T1, T2)

            # ✅ 기존 output 재사용
            base_outputs = ro.outputs

            for i in range(self.stage0_next_send_idx, T1):
                is_last = (i == T1 - 1)

                chunk_ro = copy.copy(ro)
                logger.error("sksksk wow@@")

                 # ✅ 매 iteration마다 새 dict
                chunk_mm = dict(mm)   # shallow copy (성능 OK)
                chunk_mm["0"] = thinker_sequence_embeds[i : i + 1]
                chunk_mm["24"] = thinker_hidden_states[i : i + 1]

                chunk_ro.multimodal_output = chunk_mm

                if self.queue is not None:
                    self.queue.put(chunk_ro)
...
class MultimodalOutputProcessor(VLLMOutputProcessor):
    ...
    def add_request(
        ...
        #    logger.info(f"♻️ [OutputProcessor] Request {request_id} is already active. Skipping re-registration.")
