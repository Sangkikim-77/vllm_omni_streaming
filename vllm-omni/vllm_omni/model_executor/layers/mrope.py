    ...
    def _omni_get_input_positions_tensor(
        ...
        while idx < len(src_item):
            ...
            # 2. 오디오 토큰 처리
                ...
                # 1. 연속된 오디오 토큰 개수를 셉니다. (예: 154개)
                cnt = 0
                curr = idx
                while curr < len(src_item) and src_item[curr] == audio_token_id:
                    cnt += 1
                    curr += 1
                
                # 2. 입력 리스트 갱신 (154개 추가)
                # 원본 토큰 그대로 cnt만큼 연장
                new_src_item.extend([audio_token_id] * cnt)
                
                # 3. mRoPE 압축 길이 계산 (메타데이터 활용)
                compressed_len = cnt # 기본값 (정보 없으면 1:1)
                if audio_seqlens is not None and audio_idx < len(audio_seqlens):
                    l = audio_seqlens[audio_idx]
                    # Qwen 공식: ((l-1)//2 + 1 - 2)//2 + 1
                    compressed_len = ((l - 1) // 2 + 1 - 2) // 2 + 1
                
                # 4. [핵심] 입력 개수(cnt)에 맞춰서 위치 텐서 생성
                # - 유효 구간: compressed_len 만큼
                # - 패딩 구간: 나머지 (cnt - compressed_len) 만큼
                valid_len = min(cnt, compressed_len)
                padding_len = cnt - valid_len
                
                # 4-1. 유효 위치 생성
                pos_valid = torch.arange(valid_len, dtype=torch.long).expand(3, -1) + start_idx
                
                # 4-2. 패딩 위치 생성 (필요한 경우만)
                if padding_len > 0:
                    # 마지막 유효 위치 뒤에 이어서 번호 매김
                    pos_pad = torch.arange(padding_len, dtype=torch.long).expand(3, -1) + start_idx + valid_len
                    
                    # 두 텐서를 합쳐서 cnt 길이의 단일 텐서로 만듦 -> 사이즈 불일치 원천 차단!
                    final_pos_tensor = torch.cat([pos_valid, pos_pad], dim=1)
                else:
                    final_pos_tensor = pos_valid

                # 5. 리스트에 추가
                llm_pos_ids_list.append(final_pos_tensor)
                
                # 6. 인덱스 정리
                #idx += cnt  # 154개를 한 번에 건너뜀
                audio_idx += 1
            ...
            # [중요] 루프의 마지막: 다음 토큰으로 이동
            # 추가된 토큰 수만큼 idx를 정확히 증가시킴
            added_len = len(new_src_item) - new_src_item_len
            if added_len == 0: # 무한 루프 방지 안전장치
                idx += 1
            else:
                idx += added_len
        ...
        #print(type(mrope_position_delta))
        #if llm_positions.shape[1] != len(input_tokens):
            # 만약 여기서 로그가 찍힌다면, 루프 로직 어딘가에서 토큰을 건너뛰거나 더 넣은 것입니다.
            #print(f"🚨 ALERT: Mismatch! Tokens({len(input_tokens)}) != Positions({llm_positions.shape[1]})")
        #print(f"DEBUG: context_len={context_len}, input_len={len(input_tokens)}")
        #llm_positions = llm_positions[:, context_len:seq_len]
