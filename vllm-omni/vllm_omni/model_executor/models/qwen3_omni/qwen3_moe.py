...
class Qwen3OmniMoeSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        duration = (time.time() - start_moe) * 1000
        if duration > 100: # 100ms 이상 걸리는 MoE 레이어 감시
            logger.error(f"🐢 [MOE-BLOCK-DELAY] Took {duration:.2f}ms for {hidden_states.shape[0]} tokens")
