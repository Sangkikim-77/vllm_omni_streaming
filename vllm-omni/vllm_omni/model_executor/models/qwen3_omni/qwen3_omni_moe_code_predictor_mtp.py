...
class Qwen3OmniCodePredictorAttention(nn.Module):
    ...
    def forward_relay(
        ...
        use_cache: bool = False,
        position_ids: torch.LongTensor | None = None,
    ) -> tuple[torch.Tensor, Cache | None]: # 🎯 리턴 타입 수정
        bsz, seq_len, _ = hidden_states.shape
        ...
        cos = cos.contiguous()
        sin = sin.contiguous()
        q_heads, k_heads = apply_rotary_pos_emb(q_heads, k_heads, cos, sin, unsqueeze_dim=1)
        v_heads = v.transpose(1, 2).contiguous()
        ...
        for backend_name in attention_backends:
            ...
            try:
                attention_interface = ALL_ATTENTION_FUNCTIONS[backend_name]
                # 여기서 k_heads, v_heads는 (과거+현재)가 합쳐진 상태
                attn_output, _ = attention_interface(
                    self,
                    q_heads,
                    k_heads,
...
class Qwen3OmniCodePredictorMTPLayer(nn.Module):
    ...
    def mtp_block_relay(
        ...
        use_cache: bool = False,
        position_ids: torch.LongTensor | None = None,
    ) -> tuple[torch.Tensor, Cache | None]: # 🚀 리턴 타입 수정 (Tensor -> tuple)
        # Self-attention with residual
        residual = hidden_states
        hidden_states = self.input_layernorm(hidden_states)

        hidden_states, past_key_values = self.self_attn.forward_relay(
            hidden_states, causal_mask, cos, sin, past_key_values, cache_position, use_cache, position_ids
        )
        hidden_states = residual + hidden_states
...
class Qwen3OmniCodePredictorBaseModel(nn.Module):
    ...
    def forward_relay(
        ...
        **kwargs: Any,
        ...
        if position_ids is None:
            if cache_position is not None:
                position_ids = cache_position.unsqueeze(0)  # [num_tokens]
            else:
                position_ids = torch.arange(seq_len, device=inputs_embeds.device).unsqueeze(0)  # [1, seq_len]
        else:
            position_ids = position_ids.flatten()  # Ensure [num_tokens]

        #logger.debug(f"[MTP-Base-Relay] position_ids: {position_ids} / {position_ids.shape}")
        ...
        # Create causal mask
        # [마스크 수정] 
        # 캐시를 사용할 때 seq_len이 1이면 사실상 모든 과거를 볼 수 있어야 합니다.
        if use_cache and seq_len == 1:
            causal_mask = None # 많은 백엔드에서 None은 모든 과거 참조를 허용합니다.
        else:
            device = inputs_embeds.device
            causal_mask = (
                torch.triu(torch.ones(seq_len, seq_len, dtype=torch.bool, device=device), diagonal=1)
        ...
        for layer in self.layers:
            hidden_states, past_key_values = layer.mtp_block_relay(
                hidden_states, causal_mask, cos, sin, past_key_values, cache_position, use_cache, position_ids
            )
        ...
        # Return in HF-compatible format
        Output = namedtuple("Output", ["last_hidden_state", "past_key_values"])
        return Output(last_hidden_state=hidden_states, past_key_values=past_key_values)  # [batch, num_code_groups-1, hidden_size]
...
class Qwen3OmniMoeTalkerCodePredictor(nn.Module):
    ...
    def forward_relay(
        ...
        inputs_embeds: torch.Tensor,  # [batch, seq_len, hidden_size]
        layer_idx: int,  # Which layer to predict (0-num_layers-2 for layers 1-num_layers-1)
        past_key_values: Any | None = None, # 👈 기억을 받기 위한 파라미터 추가
        **kwargs # use_cache 등 전달용
    ) -> tuple[torch.Tensor, torch.Tensor, Any | None]: # 🎯 리턴 타입에 캐시 추가
        """
        Forward pass for code predictor.
        ...
        Returns:
            ...
            hidden_states: Output hidden states [batch_size, seq_len, hidden_size]
        """
        # 1. 모델 호출 (NamedTuple을 리턴함)
        model_output = self.model.forward_relay(
            inputs_embeds=inputs_embeds, 
            past_key_values=past_key_values,
            **kwargs
        )

        # 2. 텐서와 캐시를 분리 (이게 핵심입니다!)
        # 객체 안에서 '진짜 텐서'인 last_hidden_state만 꺼냅니다.
        hidden_states = model_output.last_hidden_state 
        updated_past_key_values = model_output.past_key_values

        # 3. 이제 hidden_states는 순수한 torch.Tensor이므로 lm_head 연산이 가능합니다.
        logits = self.lm_head[layer_idx](hidden_states)

        # 4. 상위 루프를 위해 텐서 두 개와 업데이트된 캐시를 함께 리턴합니다.
        return logits, hidden_states, updated_past_key_values
