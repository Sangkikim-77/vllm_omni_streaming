...
class Request:
    ...
    def from_engine_core_request(
        ...
        # [🚨 1. 택배 상자(request)에 데이터가 들어있는지 확인]
        raw_info = getattr(request, "additional_information", None)
        
        #if raw_info:
            # 데이터가 있다면 크기나 키를 찍어봅니다.
        #    logger.info(f"📦 [REQUEST LOAD] Received additional_information! Type: {type(raw_info)}")
        #else:
            # 데이터가 없다면 여기서 유실된 것입니다.
        #    logger.info(f"❌ [REQUEST LOAD] additional_information is MISSING in EngineCoreRequest")
